...
void jointStateCallback(const sensor_msgs::JointStateConstPtr& msg) {
    auto iter = std::find(msg->name.begin(), msg->name.end(),
                          std::string{"axle_to_left_wheel"});
    ...
    iter = std::find(msg->name.begin(), msg->name.end(),
                     std::string{"axle_to_right_wheel"});
    ...
    // ROS_INFO_STREAM("right speed: " << speed_measured_right << " left speed:
    // " << speed_measured_left);
}
...
int main(int argc, char** argv) {
    ...
    ros::Publisher leftWheelEffortPublisher =
        handle.advertise<std_msgs::Float64>(
            "/igvc/left_wheel_effort_controller/command", 1);
    ros::Publisher rightWheelEffortPublisher =
        handle.advertise<std_msgs::Float64>(
            "/igvc/right_wheel_effort_controller/command", 1);
    ros::Publisher wheelSpeedPublisher =
        handle.advertise<igvc_msgs::velocity_pair>("/encoders", 1);

    auto speedSub = handle.subscribe("/motors", 1, speedCallback);

    auto stateSub =
        handle.subscribe("/igvc/joint_states", 1, jointStateCallback);
    ...
    while (ros::ok()) {
        ...
        auto effort_left =
            speed_P_left * error_left - speed_D_left * dError_left;
        ...
        auto effort_right =
            speed_P_right * error_right - speed_D_right * dError_right;
