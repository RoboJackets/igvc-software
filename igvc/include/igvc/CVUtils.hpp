...
pcl::PointXYZ PointFromPixel(const cv::Point& pixel,
                             const tf::Transform& cameraFrameToWorldFrame,
                             image_geometry::PinholeCameraModel cam) {
    cv::Point3d cameraRay = cam.projectPixelTo3dRay(pixel);
    tf::Point worldCameraOrigin =
        cameraFrameToWorldFrame * tf::Vector3(0, 0, 0);
    tf::Point worldCameraStep =
        cameraFrameToWorldFrame *
            tf::Vector3(cameraRay.x, cameraRay.y, cameraRay.z) -
        worldCameraOrigin;
...
pcl::PointXYZ PointFromPixelNoCam(const cv::Point& p, int height, int width,
                                  double HFOV, double VFOV, double origin_z,
...
double toRadians(double degrees) { return degrees / 180.0 * M_PI; }
...
pcl::PointCloud<pcl::PointXYZ>::Ptr toPointCloud(
    ...
    tf_listener.lookupTransform("/base_footprint", topic, ros::Time(0),
                                transform);
...
pcl::PointCloud<pcl::PointXYZ>::Ptr toPointCloud(
    ...
    for (std::vector<cv::Point> contour : contours) {
        for (cv::Point p : contour) {
            cloud->points.push_back(PointFromPixelNoCam(
                p, height, width, HFOV, VFOV, origin_z, origin_y, pitch));
        }
    }
