...
void coordAfterMotion(double lat1, double lon1, double d, double a,
                      double& newLat, double& newLon) {
    // angle in radians
    // dist in meters
    d = d / (EARTHRADIUS_KM *
             1000.0);  // angle corresponding to arc length/dist on earth
    ...
    if (cos(newLat) == 0)
        ...
        newLon = fmod(lon1 - asin(sin(a) * sin(d) / cos(newLat)) + M_PI,
                      2.0 * M_PI) -
                 M_PI;
 ...
 */
void coordsToMetricXY(double lat1, double lon1, double lat2, double lon2,
                      double& dX, double& dY) {
    dX =
        std::copysign(1.0, lon2 - lon1) * coordsToMeter(lat1, lon1, lat1, lon2);
    dY =
        std::copysign(1.0, lat2 - lat1) * coordsToMeter(lat1, lon1, lat2, lon1);
}
};
