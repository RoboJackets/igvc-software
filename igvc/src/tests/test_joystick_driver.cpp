...
public:
    TestJoystickDriver()
        : handle(),
          ...
          motor_sub(handle.subscribe(
              "/motors", 1, &TestJoystickDriver::motorsCallback, this)) {}
...
protected:
    ...
    bool IsNodeReady() {
        return (mock_joy_pub.getNumSubscribers() > 0) &&
               (motor_sub.getNumPublishers() > 0);
    }
...
TEST_F(TestJoystickDriver, FullForward) {
    ...
    const igvc_msgs::velocity_pair::ConstPtr& response =
        ros::topic::waitForMessage<igvc_msgs::velocity_pair>(
            motor_sub.getTopic(), ros::Duration(1));
