...
void PotholeDetector::img_callback(
    ...
    // Determine the average of blue, green, and red pixels in the entire
    // picture in order to do adaptive thresholding.
    ...
    // Find the mean and stddev of the grayscale image in order to do adaptive
    // thresholding
    cv::Mat mean;
    cv::Mat stddev;
    ...
    GaussianBlur(src_gray, src_gray, cv::Size(gaussian_size, gaussian_size), 2,
                 2);

    // Detect circles on the image
    std::vector<cv::Vec3f> circles;
    HoughCircles(src_gray, circles, CV_HOUGH_GRADIENT, 1, src_gray.rows / 8, 50,
                 10, minRadius, maxRadius);
    ...
    for (size_t i = 0; i < circles.size(); i++) {
        ...
        // Create the rectangle and an src_gray with only that region of
        // interest
        cv::Rect roiAroundCircle(xStart, yStart, xWidth, yHeight);
        cv::Mat src_gray_roiAroundCircle = src_gray(roiAroundCircle);

        // Average up all the pixels in a circle around the center of the
        // pothole.
            ...
            for (int j = -whiteSampleRadius; j <= whiteSampleRadius; j++) {
                ...
                if (i * i + j * j <= whiteSampleRadius * whiteSampleRadius) {
                    cv::Vec3b currentPixel =
                        src.at<cv::Vec3b>(j + center.y, i + center.x);
        ...
        // Filter out the circle if the average color of the image is not an
        // offset less than the average color of the center circle sample.
        ...
        // Use an offset to align with the entire src image (which has a cropped
        // sky)
        std::vector<std::vector<cv::Point>> contours;
        cv::Point offset(xStart, yStart);
        findContours(src_gray_roiAroundCircle, contours, CV_RETR_LIST,
        ...
        for (std::vector<std::vector<cv::Point>>::iterator it =
            ...
            if ((*it).size() > (unsigned int)(contourSizeThreshold)) {
                ...
                // For each contour, find the min and max X coordinates by
                ...
                // Find top and bottom points that correspond to the centerX (so
                // get the y values)
                for (cv::Point p : *it) {
                    int x = p.x;
                    int y = p.y;
                ...
                // We now have the average color of the line above and below the
                // contour.
                // Use these averages to determine if the contour is a white
                // strip on the traffic barrel.
                // Aka if there is orange above and below the contour, filter
                // it!
...
PotholeDetector::PotholeDetector(ros::NodeHandle& handle,
    ...
    _src_img = _it.subscribeCamera(topic + "/image_raw", 1,
                                   &PotholeDetector::img_callback, this);
    ...
    handle.getParam(ros::this_node::getName() + "/config/pothole/minRadius",
                    minRadius);
    handle.getParam(ros::this_node::getName() + "/config/pothole/maxRadius",
                    maxRadius);
    handle.getParam(
        ros::this_node::getName() + "/config/pothole/whiteSampleRadius",
        whiteSampleRadius);
    handle.getParam(
        ros::this_node::getName() + "/config/pothole/contourSizeThreshold",
        contourSizeThreshold);
    handle.getParam(
        ros::this_node::getName() + "/config/pothole/blueAdaptiveThreshold",
        blueAdaptiveThreshold);
    handle.getParam(
        ros::this_node::getName() + "/config/pothole/greenAdaptiveThreshold",
        greenAdaptiveThreshold);
    handle.getParam(
        ros::this_node::getName() + "/config/pothole/redAdaptiveThreshold",
        redAdaptiveThreshold);
}
