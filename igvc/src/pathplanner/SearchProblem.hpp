...
public:
    ...
    Path(const Path<StateType, ActionType>& p)
        : states(p.states), actions(p.actions) {}
    void addState(StateType state) { states.push_back(state); }
    void addAction(ActionType action) { actions.push_back(action); }
    ...
    std::list<StateType>* getStates() { return &states; }
    std::list<ActionType>* getActions() { return &actions; }
    StateType getLastState() const {
        if (states.empty())
            throw std::out_of_range("Cannot call back() on empty list.");
        return states.back();
    }
    int getNumberOfSteps() const { return actions.size(); }
