...
public:
    ...
    bool operator()(Path<StateType, ActionType>& p1,
...
template <class StateType, class ActionType,
          template <typename...> class FrontierType>
...
public:
    ...
    // Getting this to work with the current implementation of A* is possible,
    // but difficult.
    template <class StateType, class ActionType,
              template <typename...> class FrontierType>
    static auto GenericSearch(SearchProblem<StateType, ActionType>& problem)
        -> Path<StateType, ActionType> {
        using frontier_traits =
            FrontierTraits<StateType, ActionType, FrontierType>;
        ...
        while (!frontier.empty()) {
            Path<StateType, ActionType> path =
                frontier_traits::PopNextElement(frontier);
            ...
            if (expanded.find(lastState) == expanded.cend()) {
                ...
                for (auto it = legalActions.cbegin(); it != legalActions.cend();
                     ++it) {
    ...
    template <class StateType, class ActionType>
    static auto DFS(SearchProblem<StateType, ActionType>& problem)
        -> Path<StateType, ActionType> {
    ...
    template <class StateType, class ActionType>
    static auto BFS(SearchProblem<StateType, ActionType>& problem)
        -> Path<StateType, ActionType> {
    ...
    template <class StateType, class ActionType>
    static Path<StateType, ActionType> AStar(
        SearchProblem<StateType, ActionType>& problem,
        void (*expandedCallback)(const set<StateType>&)) {
        set<StateType> expanded;
        priority_queue<Path<StateType, ActionType>,
                       vector<Path<StateType, ActionType>>,
        ...
        while (!frontier.empty() && iteration < 6000) {
            ...
            if (expanded.insert(last)
                    .second)  // expanded does not contain path's last state
            {
                ...
                for (typename list<ActionType>::iterator it =
                         legalActions.begin();
