...
void waypoint_callback(const geometry_msgs::PointStampedConstPtr& msg) {
    ...
    cout << "Waypoint received. " << search_problem.Goal.x << ", "
         << search_problem.Goal.y << endl;
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber pose_sub =
        nh.subscribe("/odom_combined", 1, position_callback);
    ...
    while (ros::ok()) {
        ...
        /* Do not attempt to plan a path if the path length would be greater
         * than 100ft (~30m).
         * This should only happen when we have received either a waypoint or
         * position estimate, but not both.
         * Long paths take forever to compute, and will freeze up this node.
         */
        auto distance_to_goal =
            search_problem.Start.distTo(search_problem.Goal);
        if (!received_waypoint || distance_to_goal == 0 ||
            distance_to_goal > 60)
            continue;
