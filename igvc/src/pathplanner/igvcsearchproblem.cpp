...
bool IGVCSearchProblem::isActionValid(SearchMove& move,
    ...
    while (current < (deltat + maxODeltaT)) {
        ...
        pcl::PointXYZ searchPoint(result.x + offsetToCenter * cos(result.theta),
                                  result.y + offsetToCenter * sin(result.theta),
                                  0);
        std::vector<int> pointIdxRadiusSearch;
        std::vector<float> pointRadiusSquaredDistance;
        int neighboorsCount = kdtree.nearestKSearch(
            searchPoint, 1, pointIdxRadiusSearch, pointRadiusSquaredDistance);
...
SearchLocation IGVCSearchProblem::getResult(SearchLocation state,
    ...
    if (abs(action.W) > 1e-10) {
        ...
        result.y = c[1];
        result.theta = c[2];
        while (result.theta < 0) result.theta += 2 * M_PI;
        while (result.theta > 2 * M_PI) result.theta -= 2 * M_PI;
    } else {
        result.theta = state.theta;
        result.x =
            state.x + (sin(M_PI_2 - result.theta) * action.V * action.DeltaT);
        result.y =
            state.y + (cos(M_PI_2 - result.theta) * action.V * action.DeltaT);
    }
    return result;
}
