...
public:
    ...
    SearchLocation getStartState() { return Start; }
    ...
    double getStepCost(SearchLocation, SearchMove action) {
        ...
        // adding cost to paths close to threshold TODO: LAUNCH PARAMETERS!
        return pathLength *
               (alpha * exp((Threshold - action.distToObs) * beta) + 1);
    }

    double getHeuristicCost(SearchLocation state) { return state.distTo(Goal); }

    bool isActionValid(SearchMove& move,
