...
void LineDetector::info_img_callback(
...
void LineDetector::img_callback(const sensor_msgs::ImageConstPtr& msg) {
    ...
    cv::HoughLinesP(working, lines, 1.0, CV_PI / 180, houghThreshold,
                    houghMinLineLength, houghMaxLineGap);
    for (size_t i = 0; i < lines.size(); ++i) {
        line(fin_img, cv::Point(lines[i][0], lines[i][1]),
             cv::Point(lines[i][2], lines[i][3]), cv::Scalar(255, 255, 255), 3,
             8);
    }
    ...
    if (hasInfo) {
        ...
        cloud =
            toPointCloud(tf_listener, MatToContours(fin_img),
                         fin_img.size().height, fin_img.size().width, topic);
    }
...
LineDetector::LineDetector(ros::NodeHandle& handle, const std::string& topic)
    ...
    if (!hasInfo) {
        _src_img = _it.subscribe(topic + "/image_raw", 1,
                                 &LineDetector::img_callback, this);
    } else {
        _src_img_info = _it.subscribeCamera(
            topic + "/image_raw", 1, &LineDetector::info_img_callback, this);
    }
    _filt_img = _it.advertise(topic + "/filt_img", 1);
    _line_cloud = handle.advertise<PCLCloud>(topic + "/line_cloud", 100);

    handle.getParam(ros::this_node::getName() + "/config/line/cannyThresh1",
                    cannyThresh1);
    handle.getParam(ros::this_node::getName() + "/config/line/cannyThresh2",
                    cannyThresh2);
    handle.getParam(ros::this_node::getName() + "/config/line/houghThreshold",
                    houghThreshold);
    handle.getParam(
        ros::this_node::getName() + "/config/line/houghMinLineLength",
        houghMinLineLength);
    handle.getParam(ros::this_node::getName() + "/config/line/houghMaxLineGap",
                    houghMaxLineGap);
}
