// compile with:
// g++ `pkg-config --cflags opencv` -o tuningParameters tuningParameters.cpp
// `pkg-config --libs opencv` -std=c++11
...
void linedetect(int, void*) {
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        line(dst_img, Point(lines[i][0], lines[i][1]),
...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        cerr << "ERROR, 1 argument expected, the file path of an image" << endl;
        return -1;
    }
    ...
    createTrackbar("Low treshold - Canny", window, &cannyThresh,
                   cannyThresh_max, linedetect);
    createTrackbar("length threshold", window, &lengthThresh, lengthThresh_max,
                   linedetect);
