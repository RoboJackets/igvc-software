...
void enabledCallback(const std_msgs::BoolConstPtr& msg) { enabled = msg->data; }
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber enabled_sub =
        nh.subscribe("/robot_enabled", 1, enabledCallback);

    ros::Publisher enc_pub =
        nh.advertise<igvc_msgs::velocity_pair>("/encoders", 1000);

    std::string device_path;
    nhp.param(std::string("device"), device_path,
              std::string("/dev/igvc_motor_arduino"));
    ...
    while (ros::ok() && port.isOpen()) {
        ...
        std::string msg =
            "$" + std::to_string(enabled ? current_motor_command.left_velocity
                                         : 0.0) +
            "," + std::to_string(enabled ? current_motor_command.right_velocity
                                         : 0.0) +
            "\n";

        if (current_motor_command.left_velocity > 0 ||
            current_motor_command.right_velocity > 0)
            ROS_INFO_STREAM(msg);
        ...
        try {
            if (!ret.empty()) {
                ...
                size_t end = ret.find('\n');
                std::string leftStr =
                    ret.substr(dollar + 1, comma - dollar - 1);
                std::string rightStr =
                    ret.substr(comma + 1, comma2 - comma - 1);
