...
int main(int argc, char** argv) {
    ...
    // send a sync request and listen to response - after that the stream should
    // restart
    port.write("#oe0#oat#o1#s12");  // turn on output stream - set output type -
                                    // request synch token
    ...
    // sequence of published messgaes - should be monatomicly increasing
    int seq = 0;
    ...
    while (ros::ok() && port.isOpen()) {
        ...
        if (accelTokens.size() == 2) {
            ...
            ROS_ERROR_STREAM(
                "Improperly formatted IMU message - accelerometer data");
        }
        ...
        if (gyroTokens.size() == 2) {
            ...
            ROS_ERROR_STREAM(
                "Improperly formatted IMU message - gyroscope data");
        }
        ...
        if (rpyTokens.size() == 2) {
            ...
            ROS_ERROR_STREAM(
                "Improperly formatted IMU message - roll/pitch/yaw data");
        }
        ...
        try {
            ...
            msg.linear_acceleration.y =
                -stof(accelTokens[1]);  // must be negative - IMU defines y
                                        // positive as right
            msg.linear_acceleration.z =
                -stof(accelTokens[2]);  // must be negative - IMU defines z
                                        // positive as down
            msg.linear_acceleration_covariance = {0.04, 1e-6, 1e-6, 1e-6, 0.04,
                                                  1e-6, 1e-6, 1e-6, 0.04};

            msg.angular_velocity.x = stof(gyroTokens[0]);
            msg.angular_velocity.y = -stof(gyroTokens[1]);
            msg.angular_velocity.z = -stof(gyroTokens[2]);
            msg.angular_velocity_covariance = {0.02, 1e-6, 1e-6, 1e-6, 0.02,
                                               1e-6, 1e-6, 1e-6, 0.02};

            float yaw =
                HALF_TO_FULL_CIRCLE_ANGLE(-stof(rpyTokens[0])) * DEG_TO_RAD;
            float pitch =
                HALF_TO_FULL_CIRCLE_ANGLE(stof(rpyTokens[1])) * DEG_TO_RAD;
            float roll =
                HALF_TO_FULL_CIRCLE_ANGLE(stof(rpyTokens[2])) * DEG_TO_RAD;
            msg.orientation =
                tf::createQuaternionMsgFromRollPitchYaw(roll, pitch, yaw);
            msg.orientation_covariance = {0.0025, 1e-6, 1e-6, 1e-6,  0.0025,
                                          1e-6,   1e-6, 1e-6, 0.0025};
