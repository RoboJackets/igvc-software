...
SerialPort::~SerialPort() { port.close(); }

bool SerialPort::isOpen() { return port.is_open(); }
...
char* SerialPort::read(int numBytes) {
    ...
    for (int i = 0; i < numBytes; i++) bytes[i] = read();
    return bytes;
}
...
std::string SerialPort::readln() {
    ...
    while (true) {
        ...
        if (in == '\n')
            return line;
        if (in != '\r')  // if using return char - new line will follow - do not
                         // terminate early
            line = line + in;
    }
}

std::string SerialPort::devicePath() { return path; }
