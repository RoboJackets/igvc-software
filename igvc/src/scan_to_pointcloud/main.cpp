...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
    ...
    for (unsigned int i = 0; i < scanData.ranges.size(); i++) {
        ...
        if (range < 0.4) {
            ...
            // No close neighbors
                   ...
                   (i == (scanData.ranges.size() - 1) ||
                    abs(scanData.ranges[i + 1] - range) > 0.2)) {
            range = scanData.range_max + 1;
        }
    }
    ...
    cloud_for_pub->push_back(pcl::PointXYZ(
        -3, 0, 0));  // really solid non-hacky fix for some unknown error
...
int main(int argc, char** argv) {
    ...
    _pointcloud_pub =
        nh.advertise<pcl::PointCloud<pcl::PointXYZ>>("/scan/pointcloud", 1);
