...
scan_to_pointcloud::scan_to_pointcloud(ros::NodeHandle node_handle,
                                       ros::NodeHandle private_node_handle)
    : _pointcloud_pub{node_handle.advertise<pcl::PointCloud<pcl::PointXYZ>>(
          "/pc2", 1)},
      _pointcloud_sub{node_handle.subscribe(
          "/scan", 1, &scan_to_pointcloud::scanCallback, this)} {
...
void scan_to_pointcloud::scanCallback(
    const sensor_msgs::LaserScan::ConstPtr &msg) {
  sensor_msgs::LaserScan scanData = *msg;

  auto rangeIsValid = [&scanData](float range) {
    return !std::isnan(range) && range > scanData.range_min &&
           range < scanData.range_max;
  };
  ...
  for (unsigned int i = 0; i < scanData.ranges.size(); i++) {
    ...
    // No valid neighbors
             ...
             (i == (scanData.ranges.size() - 1) ||
              !rangeIsValid(scanData.ranges[i + 1]))) {
      range = scanData.range_max + 1;
    }
    // No close neighbors
    else if ((i == 0 || abs(scanData.ranges[i - 1] - range) > neighbor_dist) &&
             (i == (scanData.ranges.size() - 1) ||
              abs(scanData.ranges[i + 1] - range) > neighbor_dist)) {
      range = scanData.range_max + 1;
    }
  }
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_for_pub(
