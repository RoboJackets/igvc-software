    ...
    void GetImage(uint8_t* aImage) const
      {
        ...
        GetVarByRef(mDevice->image,
                    mDevice->image+GetVar(mDevice->image_count),
                    aImage);
        playerc_camera_gotimage(mDevice);
      };
    
    // DAVID: Add support for WaitForNewImage()
    /// Waits until a new image has been received.
    /// This function should only be used if you are calling
    /// @ref PlayerClient::Read() on a separate thread.
    void WaitForNewImage() const { playerc_camera_waitfornewimage(mDevice); };
    
    // DAVID: Add support for WaitForNewImage()
    /// Returns whether a new image has been received since
    /// the last call to @ref GetImage().
    bool HasNewImage() const { return playerc_camera_hasnewimage(mDevice); };
