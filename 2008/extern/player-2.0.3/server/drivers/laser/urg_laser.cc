...
int 
urg_laser::GetReadings(urg_laser_readings_t * readings, int timeout)
{
  ...
  if (ReadUntil(file, Buffer,10, timeout) < 0)
    return -1;
  ...
  if (ReadUntil(file, Buffer,2, timeout) < 0)
    return -1;
  //Buffer[2]='\0';
  //printf("Read: %s",Buffer);
  if (Buffer[0] != '0')
  ...
  for (int i=0; ; ++i)
  {
    ...
    {
      Buffer[0] = Buffer[1];
      if (ReadUntil(file, &Buffer[1],1,timeout) < 0)
        return -1;
    }
