...
GPS::GPS(const char* gps_serial_port, int gps_baud) {
...
void GPS::Run() {
	...
	// Delay cancelation requestion until a cancelation point
 ...
 * NULL-terminated.
 */
int GPS::ReadSentence(char* buf, size_t len) {
	char* ptr;
	char* ptr2;
	size_t sentlen;
	char tmp[8];
	...
	if(sentlen > len - 1) {
		printf("NMEA sentence too long (%d bytes); truncating\n", sentlen);
		sentlen = len - 1;
	}
...
int GPS::FillBuffer() {
	...
	while(numread<=0) {
		if((numread = read(gps_fd,nmea_buf+nmea_buf_len, sizeof(nmea_buf)-nmea_buf_len-1)) < 0) {			
...
int GPS::WriteSentence(const char *buf, size_t len) {
	int s;
...
char* GPS::GetNextField(char* field, size_t len, const char* ptr) {
	char* start;
	char* end;
	size_t fieldlen;
 ...
 */
int GPS::ParseSentence(const char* buf) {
	const char* ptr = buf;
	char tmp[8];
...
int GPS::ParseGPGGA(const char *buf) {
	const char *ptr = buf;
	char field[32];
	char tmp[8];
	...
	//Publish(device_addr, PLAYER_MSGTYPE_DATA, PLAYER_GPS_DATA_STATE, &data,sizeof(gps_data_t),NULL);

	return 0;
}

// Parse the GPRMC sentence, which has date/time
int GPS::ParseGPRMC(const char *buf) {
	const char *ptr = buf;
	char field[32];
	char tmp[8];
	struct tm tms;
	...
	{
		PutData(&data,sizeof(gps_data_t),0,0);
		read_count = 0;
	}
	*/
 ...
 */
int GPS::ParsePGRME(const char *buf) {
	const char *ptr = buf;
	char field[32];
	double err;
...
int GPS::ParseGPGST(const char *buf) {
	const char *ptr = buf;
	char field[32];
	double err;
	double utc;
...
int GPS::ReadSocket(char *buf, size_t len) {
	int plen;
...
void GPS::UTM(double lat, double lon, double *x, double *y) {
	// constants
...
uint32_t GPS::time_sec(void) { 
	return(data.time_sec);
}

uint32_t GPS::time_usec(void) { 
	return(data.time_usec);
}

int32_t GPS::latitude(void) { 
	return(data.latitude);
}

int32_t GPS::longitude(void) { 
	return(data.longitude);
}

int32_t GPS::altitude(void) { 
	return(data.altitude);
}

double GPS::utm_e(void) { 
	return(data.utm_e);
}

double GPS::utm_n(void) { 
	return(data.utm_n);
}

uint32_t GPS::quality(void) { 
	return(data.quality);
}

uint32_t GPS::num_sats(void) { 
	return(data.num_sats);
}

uint32_t GPS::hdop(void) { 
	return(data.hdop);
}

uint32_t GPS::vdop(void) { 
	return(data.vdop);
}

double GPS::err_horz(void) { 
	return(data.err_horz);
}

double GPS::err_vert(void) { 
	return(data.err_vert);
}
