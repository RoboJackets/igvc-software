...
void getPosition(unsigned int * time, unsigned int * position){//should interupts be disabled in this func?
	...
	if(time != NULL){//does this save any cycles?
		*time = getTime();
	}
...
*/
	//Serial.println(dtostrf(velocity, 2, 3, strbuff));

	byte * velptr = (byte*) &velocity;
	Serial.print(velptr[0], BYTE);
	Serial.print(velptr[1], BYTE);
	Serial.print(velptr[2], BYTE);
	Serial.println(velptr[3], BYTE);
}
