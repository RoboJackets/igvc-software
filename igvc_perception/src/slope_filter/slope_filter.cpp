...
TraversabilityFilter::TraversabilityFilter() : filter_chain_("grid_map::GridMap")
{
  ...
  elevation_map_sub_ = private_nh_.subscribe("/elevation_mapping/elevation_map", 1,
...
void TraversabilityFilter::elevationMapCallback(const grid_map_msgs::GridMap& message)
{
  ...
  grid_map::GridMapCvConverter::addLayerFromImage<cv::uint8_t, 1>(elevation_data_mask, "mask_original", slope_map, 0, 255, 0);

  cv::erode(elevation_data_mask, elevation_data_mask, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2*erode_radius+1, 2*erode_radius+1), cv::Point(erode_radius, erode_radius)));
  grid_map::GridMapCvConverter::addLayerFromImage<cv::uint8_t, 1>(elevation_data_mask, "mask_eroded", slope_map, 0, 255, 0);

  cv::dilate(elevation_data_mask, elevation_data_mask, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2*dilate_radius+1, 2*dilate_radius+1), cv::Point(dilate_radius, dilate_radius)));
  grid_map::GridMapCvConverter::addLayerFromImage<cv::uint8_t, 1>(elevation_data_mask, "mask_dilated", slope_map, 0, 255, 0);
