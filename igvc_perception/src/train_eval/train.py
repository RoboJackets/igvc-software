...
def train():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print("epoch: {} total train_loss: {:.4f} total val_loss: {:.4f} total val_acc: {:.4f}".format(epoch,sum(batch_metrics['train_losses']), sum(batch_metrics['val_losses']), sum(batch_metrics['val_accuracies'])/len(batch_metrics['val_accuracies'])))
        ...
        tb.add_scalar('val_acc', sum(batch_metrics['val_accuracies'])/len(batch_metrics['val_accuracies']), epoch)
...
if __name__ == '__main__':
    ...
    if not args.test:
        ...
        val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True, num_workers=1, pin_memory=True)
        lr = args.lr
