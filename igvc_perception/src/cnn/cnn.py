...
class SegmentationModel(object):
    ...
    """
    def __init__(self, camera_names, publisher_topic, resize_width,
                 resize_height, use_preexisting_segmentation, **kwargs):
        ...
        if self.use_preexisting_segmentation:
            ...
            self.graph.load_state_dict(torch.load(kwargs["model_filename"]))
        ...
        for camera_name in camera_names:
            ...
            try:
                camera_info = rospy.wait_for_message('/%s/camera_info' %
                                                     camera_name,
                                                     CameraInfo,
                                                     timeout=5)
            ...
            K = camera_info.K
            camera_info.K = (K[0] * waf, 0., K[2] * waf, 0., K[4] * haf,
                             K[5] * haf, 0., 0., 1.)

            # adjust the projection matrix.
            P = camera_info.P
            camera_info.P = (P[0] * waf, 0., P[2] * waf, 0., 0., P[5] * haf,
                             P[6] * haf, 0., 0., 0., 1., 0.)
            ...
            cam_frame_name = camera_name.replace("usb_cam_",
                                                 "") + "_cam_optical"
            ...
            self.cloud_publishers[camera_name] = rospy.Publisher(
                '%s/%s' % ("/semantic_segmentation_cloud", camera_name),
                PointCloud2,
                queue_size=1)
    ...
    def init_point_cloud_array(self, camera_name):
        ...
        cam_transform_translation = self.cam_transform_translations[
            camera_name]
        cam_transform_rotation_matrix = self.cam_transform_rotation_matrices[
            camera_name]

        world_points_array = np.empty(
            (self.resize_height, self.resize_width, 3), dtype=np.float32)
