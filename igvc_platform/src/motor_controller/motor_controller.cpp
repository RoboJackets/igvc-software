...
#include "igvc_platform/nanopb/protos/new_swerve_commands.pb.h"
...
void MotorController::cmdCallback(const igvc_msgs::velocity_quad::ConstPtr& msg)
{
...
void MotorController::sendRequest()
{
  ...
  /* allocate space for the request message to the server */
  NewRequestMessage request = NewRequestMessage_init_zero;
...
//  request.speed_r = static_cast<float>(current_motor_command_.right_velocity);
  ...
  /* encode the protobuffer */
  bool status = pb_encode(&ostream, NewRequestMessage_fields, &request);
