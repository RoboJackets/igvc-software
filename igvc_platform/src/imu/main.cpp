...
struct IMUError
{
  int accelReturn;
  int accelRawReturn;
  int angularPosReturn;
  int angularVelReturn;
  int magnetometerReturn;
  int temperatureReturn;
};
...
private:
  unsigned int seq;
  double m_headingDeviationThresholdDegree{};
  double m_yawOffsetRadians{};
  Eigen::Matrix3d m_correctionMatrix;

  std::vector<double> m_imuOrientationCorrection;

  ros::Publisher m_imu_pub;
  ros::Publisher m_imu_raw_pub;

  int initializeIMU(FreespaceDeviceId *ids);
  void readImu(IMUError &error, IMUMessage &message) const;
  void handleError(const IMUError &error) const;
  void publishImuData(const IMUMessage &message) const;
...
IMU::IMU() : seq{0} {
  ...
  igvc::getParam(nh, "imu_orientation_correction", m_imuOrientationCorrection);
  ...
  igvc::getParam(nh, "heading_dev_thresh_deg", m_headingDeviationThresholdDegree);

  // Get yaw offset for making yaw relative to East.
  igvc::getParam(nh, "yaw_offset", m_yawOffsetRadians);

  // There should be 9 elements (3x3 matrix).
  if (m_imuOrientationCorrection.size()!=9) {
    ROS_ERROR_STREAM("IMU orientation correction matrix does not have 9 elements (3x3).");
    ros::shutdown();
  }

  // Populate an Eigen matrix.
  m_correctionMatrix = Eigen::Matrix3d(m_imuOrientationCorrection.data());

  // Publish imu data without gravity for EKF localization.
  m_imu_pub = nh.advertise<sensor_msgs::Imu>("/imu", 1000);

  // Publish imu data with gravity for factor graph implementation.
  m_imu_raw_pub = nh.advertise<sensor_msgs::Imu>("/imu_raw", 1000);

  // Initialize IMU.
  FreespaceDeviceId ids[1];
  int ret = initializeIMU(ids);
  if (ret!=FREESPACE_SUCCESS) {
    ROS_ERROR_STREAM("Failed to initialize IMU (Return Code " << ret << ")");
    ros::shutdown();
  }
  ...
  while (ros::ok()) {
    ...
    IMUError error;
    IMUMessage message;
    readImu(error, message);
    handleError(error);
    publishImuData(message);
  }
}

IMU::readIMU(IMUError &error, IMUMessage &message) {
 ...
 */
int IMU::initializeIMU(FreespaceDeviceId *ids)
{
...
void IMU::readImu(IMUError &error, IMUMessage &message) const {};
void IMU::handleError(const IMUError &error) const {};
void IMU::publishImuData(const IMUMessage &message) const {};
