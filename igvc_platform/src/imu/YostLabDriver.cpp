...
void YostLabDriver::run()
{
  ...
  while (ros::ok())
  {
    ...
    {
      line_num_++;
      std::string buf = serialReadLine();
      std::vector<double> parsed_line = parseLineFromIMU(buf);
      imu_values.insert(imu_values.end(), parsed_line.begin(), parsed_line.end());
      if (parsed_line.size() == 1)
      {
        if (line_num_ == 4)
        {
          createAndPublishIMUMessage(imu_values);
        }
        else
        {
