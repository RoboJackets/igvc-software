...
*
* Copyright (C) 2013 GNU GPL (General Public License) v3.0 - Peter Bartz [http://ptrbrtz.net]
* Copyright (C) 2011-2012 GNU GPL (General Public License) v3.0 -  Quality & Usability Lab, Deutsche Telekom Laboratories, TU Berlin
*
***************************************************************************************************************/
...
/*
  ...
  "#o<params>" - Set OUTPUT mode and parameters. The available options are:
      ...
      // Angle output and sensor data output
...
const float magn_ellipsoid_center[3] = {130.747, -60.8511, -58.9468};
const float magn_ellipsoid_transform[3][3] = {{0.922284, 0.0152275, 0.00369525}, {0.0152275, 0.952080, -0.0175593}, {0.00369525, -0.0175593, 0.993517}};
...
#define GRAVITY_MS_REF(x) ((x) * 9.7951 / 256) // convert from raw accelerometer value using local ATL gravity and an assumed 256 value for 1g reading
...
void loop()
{
  ...
  if((millis() - timestamp) >= OUTPUT__DATA_INTERVAL)
  {
    ...
    else if (output_mode == OUTPUT__MODE_BOTH_CALIB)  // Output calibrate sensors and RPY
    {
      ...
      if (output_stream_on || output_single_on) output_sensors_and_angles();
    }
