...
void NavigationServer::start(NavigationServer::GoalHandle &goal_handle)
{
    ...
    {
        ROS_ERROR_STREAM_NAMED("nav_server", "Could not connect to one or more of move_base_flex actions: 'get_path', 'exe_path', 'recovery'!");
...
void NavigationServer::actionGetPathDone(const actionlib::SimpleClientGoalState &state, const mbf_msgs::GetPathResultConstPtr &result_ptr) {
...
void NavigationServer::runExePath(nav_msgs::Path path) {
    ...
    action_client_exe_path_.sendGoal(exe_path_goal,
            boost::bind(&NavigationServer::actionExePathDone,this, _1, _2),
...
void NavigationServer::actionExePathDone(const actionlib::SimpleClientGoalState &state, const mbf_msgs::ExePathResultConstPtr &result_ptr) {
...
void NavigationServer::actionRecoveryDone(const actionlib::SimpleClientGoalState &state, const mbf_msgs::RecoveryResultConstPtr &result_ptr) {
    ...
    switch(state.state_){
        case actionlib::SimpleClientGoalState::SUCCEEDED:
            ROS_DEBUG_STREAM_NAMED("nav_server", "Execution of the recovery behavior '" << *current_recovery_behavior_ << "' succeeded!");
            ROS_DEBUG_STREAM_NAMED("nav_server", "Try planning again and increment the current recovery behavior in the list.");
        ...
        case actionlib::SimpleClientGoalState::ABORTED:
            ...
            if (current_recovery_behavior_ == recovery_behaviors_.end())
            {
                ROS_DEBUG_STREAM_NAMED("nav_server","All recovery behaviors failed. Abort recovering and abort the move_base action");
