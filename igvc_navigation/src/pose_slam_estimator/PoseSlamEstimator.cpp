...
void PoseSlamEstimator::imu_callback(const sensor_msgs::ImuConstPtr& msg) {
    ...
    if (last_imu_msg_t_ == -1)
      last_imu_msg_t_ = msg->header.stamp.sec;
    
    double dt = msg->header.stamp.sec - last_imu_msg_t_;

    gtsam::Vector3 measured_acc = (
      gtsam::Vector(3) << msg->linear_acceleration.x
                       , msg->linear_acceleration.y
                       , msg->linear_acceleration.z).finished();
    gtsam::Vector3 measured_omega = (
      gtsam::Vector(3) << msg->angular_velocity.x
                       , msg->angular_velocity.y
                       , msg->angular_velocity.z).finished();
...
void PoseSlamEstimator::gps_callback(const sensor_msgs::NavSatFixConstPtr& msg) {
    ...
    geodetic_converter_.geodetic2Ned(
        static_cast<double>(msg->latitude),
        static_cast<double>(msg->longitude),
        static_cast<double>(msg->altitude),
        &north, &east, &down
    );
    ...
    gtsam::CombinedImuFactor imu_factor(
      X(correction_count_ - 1), V(correction_count_ - 1), X(correction_count_),
    ...
    gtsam::GPSFactor gps_factor (X(correction_count_),
      gtsam::Point3(north, east, down), correction_noise);
    ...
    prev_state_ = gtsam::NavState(
      results.at<gtsam::Pose3>(X(correction_count_)),
      results.at<gtsam::Vector3>(V(correction_count_)));
    prev_bias_ = results.at<gtsam::imuBias::ConstantBias>(B(correction_count_));
...
void PoseSlamEstimator::initialize_pose_graph() {
  ...
  gtsam::Rot3 prior_rotation = gtsam::Rot3::Quaternion(initial_state_(6), initial_state_(3), 
                                         initial_state_(4), initial_state_(5));
  ...
  initial_values.insert(B(correction_count), prior_imu_bias_);

  // Anchor the factor graph by adding all prior factors.
  graph_->add(gtsam::PriorFactor<gtsam::Pose3>(
    X(correction_count_), prior_pose, pose_noise_model_));
  graph_->add(gtsam::PriorFactor<gtsam::Vector3>(
    V(correction_count_), prior_velocity, velocity_noise_model_));
  graph_->add(gtsam::PriorFactor<gtsam::imuBias::ConstantBias>(
    B(correction_count_), prior_imu_bias_, bias_noise_model_));
