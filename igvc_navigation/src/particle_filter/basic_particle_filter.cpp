 ...
 */
void BasicParticleFilter::proposal_distribution(const RobotState& state,
                                                const igvc_msgs::velocity_pairConstPtr& motor_command)
{
  ...
  for (Particle& particle : particles)
  {
    ...
    if (fabs(left_delta - right_delta) < 1.0e-6)
    {
      ...
      particle.state.x += left_delta * cos(state.yaw);
      particle.state.y += right_delta * sin(state.yaw);
    }
    else
    {
      double r = axle_length * (left_delta + right_delta) / (2 * (right_delta - left_delta));
      double wd = (right_delta - left_delta) / axle_length;
      particle.state.x += r * sin(wd + state.yaw) - r * sin(state.yaw);
      particle.state.y += -r * cos(wd + state.yaw) + r * cos(state.yaw);
