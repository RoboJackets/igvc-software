...
//Populate coordinates (lat, lon, lat, etc.)
...
#pragma omp parallel for private(curPathDanger,curPixelDanger,weight,pathID)
	...
	/* Pick a path with a low danger-value to follow */
	{
		...
		int bestPath_id = 0;
		{
			...
			int bestPath_distanceFromCenter = abs(nav_path__chosen_path_id - bestPath_id);
			int curPath_danger;
			...
			for (int curPath_id=0; curPath_id<nav_path__num; curPath_id++)
			{
				...
				curPath_distanceFromCenter = abs(nav_path__chosen_path_id - curPath_id);
