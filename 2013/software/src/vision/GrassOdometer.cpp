...
void GrassOdometer::ProcesImageSURF(Mat& frame)
{
  ...
  for(int i=0;i<keypoints.size();i++)
  {
    ...
    pos = ImgRollCorrectMat(roll)*pos; //Correct for roll of image by rotating it back the opposite way
  ...
  currentPos = rotDynMat.inverse()*currentPos; //makes positions extrinsic. i.e. y is now north, etc
  //Match points
...
Matrix3d GrassOdometer::RollRotMatrix(int roll)
{
...
Matrix4d GrassOdometer::HomogRollRotMatrix(int roll)
{
...
Matrix3d GrassOdometer::PitchRotMatrix(int pitch)
{
   Matrix3d rMat = MatrixXd::Zero(3,3);
   rMat(0,0) = cos(pitch);
   rMat(0,2) = sin(pitch);
...
Matrix4d GrassOdometer::HomogPitchRotMatrix(int pitch)
{
...
Matrix3d GrassOdometer::YawRotMatrix(int yaw)
{
...
Matrix4d GrassOdometer::HomogYawRotMatrix(int yaw)
{
...
Matrix3d GrassOdometer::RotMat3d(int roll, int pitch, int yaw)
{
  return YawRotMatrix(yaw)*PitchRotMatrix(pitch)*RollRotMatrix(roll);
}

Matrix4d GrassOdometer::HomogRotMat3d(int roll, int pitch, int yaw)
{
...
Matrix3d GrassOdometer::centerImageCoords(int nRows, int nCols)
{
  ...
  transMat(0,3) = -(nRows-1)/2;
  transMat(1,3) = -(nCols-1)/2;
  return transMat;
}
