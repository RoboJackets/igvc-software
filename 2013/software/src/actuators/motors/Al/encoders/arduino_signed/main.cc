	...
	for(;;)
	{
		...
		switch(header.cmd)
		{
			...
			case ENCODER_GET_READING:
			{
				...
				headerOut.packetnum = tx_num;
				headerOut.cmd = ENCODER_GET_READING;
				headerOut.size = sizeof(new_encoder_single_pk_t);

				new_encoder_single_pk_t body;

				noInterrupts();
				int64_t first = coder_ticks;
				interrupts();
				_delay_ms(5);
				noInterrupts();
				body.pcoder = coder_ticks;
				interrupts();

				body.dcoder = body.pcoder - first;

				uint8_t* msg = (uint8_t*)&(body);
				Serial.write((uint8_t*)&headerOut, PACKET_HEADER_SIZE);
				Serial.write(msg, sizeof(new_encoder_single_pk_t));
				tx_num++;
				break;
			}
			#endif
