...
#ifndef useMotors//deprecated, now using useMotors and useEncoders
...
void OSMC_driver::moveDistance(const int dist, const int speed)
{
	...
	int motorPwm = speed * MINREQSPEED;
...
*/
	...
	double dist_l;
	double dist_r;
	if (getEncoderDist(dist_l,dist_r) ==  true)
	{
		if (((dist_l+dist_r)/2) >= dist)
		{
			set_motors(0);
		}
		else
