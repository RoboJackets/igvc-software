...
void OSMC_4wd_driver::setVel_pd(const double FR, const double FL, const double BR, const double BL)
{
	FOR.setVel_pd(FL, FR);
	AFT.setVel_pd(BL, BR);
}

void OSMC_4wd_driver::setVel_pd(const double v)
{
	setVel_pd(v,v,v,v);
}
...
//move the robot a distance in meters at a certain speed in mph, use negative speed values for backwards
void OSMC_4wd_driver::moveDistance(const int dist, const int speed)
{
	//so we cant accidently go faster than roughly 4 mph
	if (speed > 4)
	{
		speed = 1;
	}
	int motorPwm = speed * 30;	//30 is MINREQSPEED
/*	if (motorPwm > _max_speed_)
	{
		motorPwm = _max_speed_;
	}
*/
	set_motors(motorPwm);
	double FR;
	...
	double BL;
	double BR;
	if (getEncoderDist(FL,FR,BL,BR) ==  true)
	{
		if (((FL+FR+BL+BR)/4) >= dist)
		{
			set_motors(0);
		}
		else
	...
	{
		std::cout<<"Encoder Fail"<<std::endl;
	}
}
